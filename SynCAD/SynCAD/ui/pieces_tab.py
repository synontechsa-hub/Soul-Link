        ...
        try:
            with open(path, newline="", encoding="utf-8-sig") as f:
                ...
                for row in reader:
                    ...
                    qty = int(row.get("qty", row.get(
                        "quantity", row.get("Qty", 1))))
                    w = int(row.get("width", row.get("Width", row.get("w", 0))))
                    h = int(row.get("height", row.get(
                        "Height", row.get("h", 0))))
                    label = row.get("label", row.get(
                        "Label", row.get("name", "")))
                    rotate = str(row.get("can_rotate", row.get(
                        "rotate", "yes"))).lower() not in ("no", "0", "false")
                    if w > 0 and h > 0:
                        self._add_piece(
                            Piece(quantity=qty, width=w, height=h, label=label, can_rotate=rotate))
                        count += 1
            QMessageBox.information(
                self, "Import Done", f"Imported {count} piece(s).")
