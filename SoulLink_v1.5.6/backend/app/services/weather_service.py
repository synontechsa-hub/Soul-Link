...
# ── Load weather.json once at import time (no DB, no repeated I/O) ──────────
...
try:
    ...
    logger.info("WeatherService: weather.json loaded (%d weather types)", len(
        _WEATHER_DATA["weather_types"]))
except FileNotFoundError:
    logger.warning(
        "WeatherService: weather.json not found at %s — using hardcoded fallback", _WEATHER_DATA_PATH)
    _WEATHER_DATA = None

# Pre-index weather types by id for O(1) lookup
_WEATHER_BY_ID: dict = {}
if _WEATHER_DATA:
    _WEATHER_BY_ID = {w["id"]: w for w in _WEATHER_DATA["weather_types"]}
