...
import 'apartment_screen.dart'; // Added import
...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _moveUserTo(
    ...
    try {
      ...
      // 🏠 SPECIAL HANDLING: If moving to Apartment, navigate there!
      // This prevents the "Black Screen" by showing the actual room.
      if (locationId == 'user_loft' ||
          locationId == 'linkside_apartment' ||
          locationId == 'linkside_estate') {
        Navigator.push(
          context,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("IDENTITY TRANSMARKED: YOU ENTERED $locationName"),
          backgroundColor: Colors.green,
        ),
      );
      provider.refreshUser();
    } catch (e) {
      if (mounted && Navigator.canPop(modalContext)) {
        Navigator.pop(modalContext);
      }
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("USER MOVE FAILED: $e")));
    }
  }

  void _showLocationDetails(dynamic loc, List<SoulRelationship> allSouls) {
    final soulsHere = allSouls
        .where((s) => s.currentLocation == loc['location_id'])
        .toList();
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            // LOCATION HEADER IMAGE (New)
            if (loc['image_url'] != null &&
                loc['image_url'].toString().isNotEmpty)
              Container(
                height: 150,
                ...
                decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                    image: NetworkImage(
                      AppConfig.getImageUrl(loc['image_url']),
                    ),
                    fit: BoxFit.cover,
                  ),
                ),
            ...
            Text(
              (loc['display_name'] ?? loc['name'] ?? 'UNKNOWN')
                  .toString()
                  .toUpperCase(),
            ...
            Text(
              loc['desc'] ?? "Scanning sector... Signal strength nominal.",
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, i) => Padding(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundImage: (soulsHere[i].portrait_url.isNotEmpty)
                          ? NetworkImage(
                              AppConfig.getImageUrl(soulsHere[i].portrait_url),
                            )
                          : null,
                      child: soulsHere[i].portrait_url.isEmpty
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          if (state != null &&
              dashboard.currentUser?.accountTier == 'architect')
            RadarSelector(
