...
class ApiService {
  ...
  Map<String, String> get _headers {
    ...
    if (token != null)
      print("🔑 API Call with Token: ${token.substring(0, 10)}...");
  ...
  Future<Map<String, dynamic>> sendMessage(
    ...
    final response = await http.post(
      ...
      headers: _headers,
      body: jsonEncode({"soul_id": soulId, "message": message}),
    );
  ...
  Future<Map<String, dynamic>> getUserProfile() async {
    try {
      ...
      print(
        "❌ PROFILE ERROR: Status=${response.statusCode}, Body=${response.body}",
      );
  ...
  Future<List<dynamic>> getMapLocations() async {
    final response = await http.get(
      Uri.parse("$baseUrl/map/locations"),
      headers: _headers,
    );
    if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<List<dynamic>> getExploreSouls() async {
    final response = await http.get(
      Uri.parse("$baseUrl/souls/explore"),
      headers: _headers,
    );
    if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> linkWithSoul(String soulId) async {
    final response = await http.post(
      Uri.parse("$baseUrl/souls/$soulId/link"),
      headers: _headers,
    );
    if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> moveSoul({
  ...
  Future<Map<String, dynamic>> advanceTimeSlot({String? targetSlot}) async {
    final response = await http.post(
      ...
      headers: _headers,
      body: jsonEncode({if (targetSlot != null) "target_slot": targetSlot}),
    );
    if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> getAdConfig() async {
    final response = await http.get(
      Uri.parse("$baseUrl/ads/config"),
      headers: _headers,
    );
    if (response.statusCode == 200) return jsonDecode(response.body);
    throw Exception("Ad Config Error");
  }

  Future<Map<String, dynamic>> verifyAdReward({
    required String rewardType,
    required int rewardAmount,
    String adNetwork = "mock_network",
    String? signature,
  }) async {
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/ads/verify-reward"),
      headers: _headers,
      body: jsonEncode({
        "user_id": uid,
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception("Ad Verification Failed: ${response.body}");
    }
  }

  Future<Map<String, dynamic>> getUserStability(String? soulId) async {
    final uri = Uri.parse(
      "$baseUrl/user/stability",
    ).replace(queryParameters: soulId != null ? {"soul_id": soulId} : null);
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception("Stability Fetch Failed: ${response.body}");
    }
  }

  Future<Map<String, dynamic>> toggleNsfw(String soulId, bool enabled) async {
    final response = await http.post(
      Uri.parse("$baseUrl/user/nsfw-toggle"),
      headers: _headers,
      body: jsonEncode({"soul_id": soulId, "nsfw_enabled": enabled}),
    );

    if (response.statusCode == 200) {
      return jsonDecode(response.body);
    } else {
      throw Exception("NSFW Toggle Failed: ${response.body}");
    }
  }
}
