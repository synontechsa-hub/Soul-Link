...
def validate():
    ...
    with Session(engine) as session:
        ...
        pillars = session.execute(
            text("SELECT COUNT(*) FROM soul_pillars")).scalar()
        states = session.execute(
            text("SELECT COUNT(*) FROM soul_states")).scalar()
        locations = session.execute(
            text("SELECT COUNT(*) FROM locations")).scalar()
        ...
        all_pass &= check(
            "Souls with interaction_system populated", souls - missing_ie, souls)
        all_pass &= check("Souls with meta_data populated",
                          souls - missing_meta, souls)
        ...
        all_pass &= check("Locations missing privacy_gate", no_gate, 0) if no_gate == 0 else (
            print(f"  [FAIL] {no_gate} locations missing privacy_gate!") or False)
        ...
        if amber_p:
            all_pass &= check("Amber Interaction System",
                              1 if amber_p.interaction_system else 0, 1)
            all_pass &= check("Amber Meta Data",
                              1 if amber_p.meta_data else 0, 1)

            recog = amber_p.meta_data.get("recognition_protocol", {})
            all_pass &= check("Amber Architect Awareness",
                              1 if recog.get('architect_awareness') else 0, 1)
        ...
        if arch_uuid:
            ...
            all_pass &= check(
                f"Souls recognising architect ({arch_uuid[:8]}...)", arch_in_db, souls)
