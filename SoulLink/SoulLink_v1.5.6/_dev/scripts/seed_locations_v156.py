...
def seed():
    files = [f for f in LOCATIONS_DIR.glob(
        "*.json") if not f.name.startswith("_")]
    ...
    with Session(engine) as session:
        for fpath in sorted(files):
            try:
                ...
                privacy = data.get("system_modifiers", {}).get(
                    "privacy_gate", "Public")

                if DRY_RUN:
                    print(
                        f"  [PREVIEW] {loc_id} -> {meta.get('display_name')} [{privacy}]")
                    success += 1
                    continue
                ...
                loc_data = dict(
                    ...
                    image_url=meta.get(
                        "image_url", f"/assets/images/locations/{loc_id}_01.jpg"),
                    music_track="ambient_city_loop.mp3",
                    system_modifiers=data.get(
                        "system_modifiers", {"privacy_gate": "Public"}),
                    system_prompt_anchors=data.get(
