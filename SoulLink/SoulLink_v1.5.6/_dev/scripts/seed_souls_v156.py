        ...
        for fpath in sorted(files):
            ...
            try:
                ...
                if architect_uuid and architect_uuid not in dev_config.get("architect_ids", []):
                    dev_config.setdefault(
                        "architect_ids", []).append(architect_uuid)
                meta["dev_config"] = dev_config

                # ── Upsert Soul ──────────────────────────────────────────────
                identity = data.get("identity", {})
                soul_name = identity.get("name", data.get("name", soul_id))
                soul_summary = identity.get("summary", data.get(
                    "short_description", "A mysterious soul..."))[:500]

                existing_soul = session.get(Soul, soul_id)
                if existing_soul:
                    existing_soul.name = soul_name
                    existing_soul.summary = soul_summary
                    existing_soul.portrait_url = data.get("aesthetic", {}).get(
                        "portrait_path", f"/assets/images/souls/{soul_id}_01.jpeg")
                    existing_soul.archetype = identity.get(
                    ...
                    soul = Soul(
                        ...
                        portrait_url=data.get("aesthetic", {}).get(
                            "portrait_path", f"/assets/images/souls/{soul_id}_01.jpeg"),
                        archetype=identity.get(
                            "archetype_id", data.get("archetype")),
                ...
                if not hasattr(seed, 'routines_lib'):
                    routines_path = DATA_DIR / "system" / "routines.json"
                    try:
                ...
                if template:
                    ...
                    for slot, zone_key in weekday_sched.items():
                        # Fallback to key if no pref
                        resolved_loc = prefs.get(zone_key, zone_key)
                ...
                home_zone = routine_data.get("location_preferences", {}).get(
                    "home_zone", "soul_plaza")
