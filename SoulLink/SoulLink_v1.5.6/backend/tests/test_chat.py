...
from backend.app.models import User, Soul, LinkState, SoulPillar, SoulState
...
async def setup_chat_data(async_session):
    ...
    try:
        # 1. Create User
        user = User(user_id=TEST_USER_ID, username="Tester",
        ...
        soul = Soul(id=TEST_SOUL_ID, soul_id=TEST_SOUL_ID,
                    name="Test Soul", slug="test-soul")
        async_session.add(soul)
        await async_session.commit()  # Commit parent first

        # 3. Create Pillar (Required for Logic)
        pillar = SoulPillar(soul_id=TEST_SOUL_ID,
                            name="Logic Core", routines={})
        ...
        state = SoulState(soul_id=TEST_SOUL_ID,
...
async def test_send_chat_message(client: AsyncClient, setup_chat_data, async_session):
    ...
    with patch("backend.app.logic.brain.LegionBrain.generate_response") as mock_brain:
        ...
        mock_user = User(user_id=TEST_USER_ID,
                         username="Tester", account_tier="architect")
...
async def test_chat_invalid_soul(client: AsyncClient):
    ...
    app.dependency_overrides[get_current_user] = lambda: User(
        user_id=TEST_USER_ID, account_tier="free")
...
async def test_get_chat_history_empty(client: AsyncClient):
    """GET /chat/history returns empty list when no messages exist."""
    from backend.app.api.dependencies import get_current_user
    from backend.app.main import app
    app.dependency_overrides[get_current_user] = lambda: User(
        user_id=TEST_USER_ID, account_tier="free")

    response = await client.get(f"/api/v1/chat/history?soul_id={TEST_SOUL_ID}")
    assert response.status_code == 200
    assert response.json() == []


@pytest.mark.asyncio
async def test_get_chat_history_returns_messages(client: AsyncClient, setup_chat_data, async_session):
    """GET /chat/history returns seeded messages with correct shape."""
    from backend.app.api.dependencies import get_current_user
    from backend.app.main import app
    ...
    assert data[0]["content"] == "Hello, test world!"
