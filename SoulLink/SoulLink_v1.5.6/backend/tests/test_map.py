...
from backend.app.models import User, Soul, LinkState, Location
...
async def test_get_world_state(client: AsyncClient, setup_map_data):
    ...
    app.dependency_overrides[get_current_user] = lambda: User(
        user_id=ARCHITECT_ID, account_tier="architect")
...
async def test_move_soul_forbidden(client: AsyncClient, setup_map_data):
    ...
    # Mock Free User
    app.dependency_overrides[get_current_user] = lambda: User(
        user_id=FREE_USER_ID, account_tier="free")
...
@pytest.mark.asyncio
async def test_get_location_narration(client: AsyncClient, setup_map_data):
    """Test the /narrate endpoint returns correct structured narration data."""
    from datetime import datetime
    from backend.app.models.user import User as UserModel
    from unittest.mock import patch, AsyncMock

    # Build a mock user with full world-state fields
    mock_user = UserModel(
        user_id=ARCHITECT_ID,
        username="architect",
        account_tier="architect",
    ...
    # Mock TimeManager.get_world_state to return a controlled world state
    mock_world_state = {
        "time_slot": "morning",
        "soul_locations": {},  # No souls at this location for simplicity
        "season": "frostlink",
        "weather": "clear_frost",
    }

    with patch(
        "backend.app.api.map.TimeManager.get_world_state",
        new_callable=AsyncMock,
        return_value=mock_world_state
    ):
        response = await client.get(f"/api/v1/map/locations/{LOCATION_ID_1}/narrate")

    assert response.status_code == 200
    data = response.json()

    # Verify the contract: all required keys are present
    assert "location_id" in data
    assert "base_description" in data
    assert "music_track" in data
    assert "environmental_factors" in data
    assert "present_souls" in data

    # Verify environmental_factors has real weather fields (not just time_slot)
    env = data["environmental_factors"]
    assert "season" in env
    assert "weather" in env
    assert "time_slot" in env

    # Verify the values are real strings — not empty
    assert env["season"]
    assert env["weather"]

    # Verify location_id echoes back correctly
    assert data["location_id"] == LOCATION_ID_1
