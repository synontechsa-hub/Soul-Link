...
async def lifespan(app: FastAPI):
    ...
    try:
        async with async_session_maker() as session:
            # ── 1. Soul health check (DB-based, no _dev/ needed) ────────────
            ...
            # ── 3. Load routines cache from system_config ────────────────────
            ...
            if routines_cfg and routines_cfg.data:
                initialize_routines_cache(routines_cfg.data)
            else:
                logger.warning(
                    "⚠️  routines not in system_config — soul positions may be incorrect")

            # ── 4. Warm World State Cache ────────────────────────────────────
...
def read_root():
    if settings.environment == "production":
        return {"status": "ok", "version": VERSION_SHORT}
    return {
