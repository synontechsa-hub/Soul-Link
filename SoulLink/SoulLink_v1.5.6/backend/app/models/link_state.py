...
class LinkState(SQLModel, table=True):
    ...
    __table_args__ = (UniqueConstraint(
        "user_id", "soul_id", name="uq_link_state_user_soul"),)
    ...
    current_location: Optional[str] = Field(
        default=None, max_length=50, description="Override location for this user only")
    energy_pool: int = Field(
        default=100, description="User-specific energy interaction pool")
    ...
    mask_integrity: float = Field(
        default=1.0, description="1.0 = Full Persona, 0.0 = Raw Soul/Glitch")
    ...
    total_messages_sent: int = Field(
        default=0, description="Total messages exchanged in this link")
