...
class TimeManager:
    ...
    async def advance_time_slot(self, user_id: str, target_slot: Optional[TimeSlot] = None) -> TimeSlot:
        ...
        new_slot = target_slot if target_slot else get_next_time_slot(
            current_slot)
        ...
        try:
            ...
            logger.error(
                "Failed to commit time slot change: %s", e, exc_info=True)
            raise

        # 🚀 REAL-TIME: Send time update to specific user only AFTER commit succeeds
    ...
    async def get_world_state(self, user_id: str, current_slot: Optional[TimeSlot] = None) -> Dict:
        ...
        if cached_data:
            # New dict — never mutate shared cache
        ...
        rel_result = await self.session.execute(
            select(LinkState).where(  # type: ignore
                LinkState.user_id == user_id,
                LinkState.current_location != None  # type: ignore
            )
        )
    ...
    async def warm_world_state_cache(self):
        ...
        logger.info(
            f"🔥 Warming World State Cache for {len(soul_ids)} souls...")
        ...
        pillar_result = await self.session.execute(
            select(SoulPillar.soul_id, SoulPillar.routine,
                   SoulPillar.routines)  # type: ignore
        )
        pillar_map = {row[0]: {"routine": row[1], "routines": row[2]}
                      for row in pillar_result.all()}
        ...
        for slot in TimeSlot:
            ...
            logger.info(
                f"   ✅ Cached {slot.value}: {len(locations)} locations (In-Memory Fast Path)")
