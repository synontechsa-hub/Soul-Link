...
from backend.app.services.response_engine import ResponseEngine
...
class LegionBrain:
    ...
    async def generate_response(
        ...
        # 3. LORE INJECTION (ResponseEngine)
        try:
            dynamic_lore = ResponseEngine.inject_lore(
                soul_id, user_input, pillar, link_state)
            if dynamic_lore:
                world_state_injection = f"{world_state_injection}\n{dynamic_lore}".strip(
                )
        except Exception as e:
            logger.error("❌ RESPONSE ENGINE FAILED: %s", e)

        # 4. BUILD PROMPT (Delegated to ContextAssembler)
        try:
