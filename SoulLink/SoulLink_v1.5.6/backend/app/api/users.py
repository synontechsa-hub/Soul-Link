...
class UserProfile(BaseModel):
    ...
    nsfw_enabled: bool  # The Global NSFW Toggle
    # v1.5.6 Active persona (null if using default)
...
async def activate_persona(
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="Persona not found or does not belong to user.")
...
async def update_profile(
    ...
    if data.nsfw_enabled is not None:
        # Gate: NSFW requires explicit age verification (18+)
        if data.nsfw_enabled is True and (user.age is None or user.age < 18):
            raise HTTPException(
                status_code=403,
                detail="NSFW content requires age verification (18+)."
            )
        user.nsfw_enabled = data.nsfw_enabled
