...
async def explore_souls(
    ...
    try:
        ...
        for s in all_souls:
            # Note: The Architect is a lore entity only — no Soul DB row exists for them,
        ...
        raise HTTPException(
            status_code=500, detail="Failed to scout for souls.")
...
async def get_soul_details(soul_id: str, db: AsyncSession = Depends(get_async_session)):
    ...
    if not soul or not pillar:
        raise HTTPException(
            404, detail=f"Soul {soul_id} not fully initialized.")
...
async def link_with_soul(
    ...
    if not soul or not pillar or not state:
        raise HTTPException(
            404, detail=f"Soul {soul_id} definition incomplete.")
    ...
    dev_cfg = pillar.meta_data.get(
        "dev_config", {}) if pillar.meta_data else {}
    allowed_ids = dev_cfg.get("architect_ids", [])
    is_architect = (user.user_id == settings.architect_uuid) or (
        user.user_id in allowed_ids)
    ...
    logger.info(
        f"Link established: user={user.user_id} soul={soul_id} architect={is_architect}")
...
async def get_relationship_status(
    ...
    if not link:
        if user.user_id == settings.architect_uuid:
        ...
        raise HTTPException(
            404, detail=f"No link with soul '{soul_id}'. Link with this soul first.")
