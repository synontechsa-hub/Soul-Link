...
async def verify_ad_reward(
    ...
    if settings.applovin_ssv_secret == "mock_secret" and getattr(settings, "environment", "development") == "production":
        logger.critical(
            "Security Breach Attempt: SSV endpoint called in production with mock secret.")
        raise HTTPException(
            status_code=500, detail="SSV verification cannot run with mock keys in production."
        )
