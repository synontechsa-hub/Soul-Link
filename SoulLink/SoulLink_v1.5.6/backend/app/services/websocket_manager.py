...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str) -> None:
        ...
        if user_id in self._connections:
            if len(self._connections[user_id]) >= self.MAX_CONNECTIONS_PER_USER:
                ...
                logger.warning(
                    f"FIFO Eviction: Disconnected oldest for {user_id}")
        ...
        logger.info(
            f"✅ WebSocket connected: {user_id} (Total: {self.get_connection_count()})")
    ...
    def disconnect(self, websocket: WebSocket) -> Optional[str]:
        ...
        if user_id and user_id in self._connections:
            ...
            logger.info(
                f"❌ WebSocket disconnected: {user_id} (Total: {self.get_connection_count()})")
